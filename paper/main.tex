...
\IEEEauthorblockA{Department of Electrical Engineering, Universitas Indonesia\\
...
Table~\ref{tab:comparison} highlights the key differences between Aegis and these related solutions, showing that Aegis occupies a unique niche by providing a practical, end-to-end DApp with robust, contribution-weighted milestone voting.
...
\begin{itemize}
	...
	\item \textbf{Incentive Alignment}: Tying vote weights to contributions aligns decision-making authority with financial exposure. Backers who have funded a larger portion of the project goal carry higher financial risk and thus command a larger voice in progress validation.
...
The frontend interface is implemented as a single-page React application using Vite for build optimization. Communication with the Ethereum network is established using Ethers.js (v6.16) and MetaMask as the EOA wallet provider. 
...
We evaluated the Aegis platform on a local Hardhat developer node. The setup included:
\begin{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Milestone Rejection \& Refund}: A creator submitted a milestone report. Backers representing 60\% of the contribution voted "No". The milestone status transitioned to `Rejected`. The creator cancelled the project. Backers successfully claimed their proportional refunds, verifying that the mathematical model in Equation (7) correctly distributed the remaining escrow balance.
...
\begin{itemize}
	...
	\item \textbf{Voting Plutocracy}: Contribution-weighted voting can lead to a plutocratic governance structure, where a single whale backer can override the votes of all smaller contributors. Mitigating this requires exploring quadratic voting or delegated governance tokens.
...
\begin{enumerate}
	...
	\item Implementing quadratic voting or hybrid governance tokens to mitigate plutocratic bias.
	\item Supporting stablecoin-based escrows and integrating decentralized finance (DeFi) yield-bearing pools to keep idle escrow funds productive.
\end{enumerate}
